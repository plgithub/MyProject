...
public class CartoonFragment extends BaseFragment implements
		OnHeaderRefreshListener, OnFooterRefreshListener, OnItemClickListener, OnClickListener,MoreMenuListAdapter.TestInterFace,
	...
	private void horizonlistviewInit() {
		...
		mainClassifyRequestManager.setTagName(AppConfig.mainClassifyNames[3]);
		requestUrl = mainClassifyRequestManager.requestUrl(0);
		filmHorizonalAdapter = new FilmHorizonalAdapter(getActivity());
		if (CommonUtil.isMainClassifyMapContainsKey(TypesManager.mainTypes[2])) {
			map = AppConfig.classifyMap.get(TypesManager.mainTypes[2]);
