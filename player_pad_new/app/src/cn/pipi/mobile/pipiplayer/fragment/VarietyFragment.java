...
public class VarietyFragment extends BaseFragment implements
		OnHeaderRefreshListener, OnFooterRefreshListener, OnItemClickListener, OnClickListener,
		MoreMenuListAdapter.TestInterFace, OnCheckedChangeListener, OnTouchListener{
	...
	public void widgetInit() {
		...
		moreMenuRadioGroup = (RadioGroup) view
				.findViewById(R.id.film_moremenurgroup);
		moreMenuRadioGroup.setOnCheckedChangeListener(this);
		horizontalListView = (HorizontalListView) view
				.findViewById(R.id.film_horizonlistview);
		horizontalListView.setOnItemClickListener(this);
		mPullToRefreshView = (PullToRefreshView) view
				.findViewById(R.id.film_pulltorefreshview);
	...
	private void horizonlistviewInit() {
		...
		mainClassifyRequestManager.setTagName(AppConfig.mainClassifyNames[2]);
		requestUrl = mainClassifyRequestManager.requestUrl(0);
		filmHorizonalAdapter = new FilmHorizonalAdapter(getActivity());
		if (CommonUtil.isMainClassifyMapContainsKey(TypesManager.mainTypes[3])) {
			map = AppConfig.classifyMap.get(TypesManager.mainTypes[3]);
...
//			list.add(0, typesBean);
	...
	public void requestUrlInit() {
		isLoading = true;
		requestUrl = mainClassifyRequestManager.requestUrl(0);
		ClassifyAsync classifyAsync = new ClassifyAsync(
	...
	public void onFooterRefresh(PullToRefreshView view) {
		if (!isClassify) {
			...
			ClassifyAsync classifyAsync = new ClassifyAsync(ASYNC_FOOT_REFRESH,
					mainClassifyRequestManager);
			classifyAsync.setContext(getActivity());
			classifyAsync.setLoadingView(loadingLayout);
			classifyAsync.setHandler(handler);
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message message) {
			...
			case MessageMark.OKAY:
				filmAdapter = new HomePagerAdapter(mContext);
...
//				filmAdapter.notifyDataSetChanged();
			...
			case ASYNC_FOOT_REFRESH: // 下拉加载更多操作
				if (filmAdapter.getList() != null
						&& filmAdapter.getList().size() != 0) {
	...
	public void onClick(View view) {
		...
		case R.id.film_more:
			if(getActivity()!=null)
				MobclickAgent.onEvent(getActivity(), "CategorySort");
			showOrHideMoreMenu();
			break;
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		if (arg0 == horizontalListView) {
			...
			if (position != 0) {
				...
				itemClassifyAsync.execute(filmHorizonalAdapter.getList()
						.get(position).getTypesID());
			} else {// 全部
				isClassify = false;
				ClassifyAsync temp = new ClassifyAsync(
						mainClassifyRequestManager);
